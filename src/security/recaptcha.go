package security

import (
	"encoding/json"
	"io/ioutil"
	"net/http"
	"net/url"
)

// RecaptchaChecker represents a function that checks recaptcha request
type RecaptchaChecker func(response string) (bool, error)

// InitRecaptchaChecker inits a service that sends request to https://www.google.com/recaptcha/api/siteverify and checks if a token is valid
func InitRecaptchaChecker(secret string) RecaptchaChecker {
	return func(response string) (bool, error) {
		resp, err := http.PostForm("https://www.google.com/recaptcha/api/siteverify", url.Values{"secret": {secret}, "response": {response}})

		if err != nil {
			return false, err
		}

		body, err := ioutil.ReadAll(resp.Body)

		if err != nil {
			return false, err
		}

		var dat map[string]interface{}
		if err := json.Unmarshal(body, &dat); err != nil {
			return false, err
		}

		return dat["success"].(bool), nil
	}
}
